...
CREATE TABLE restaurante.pedido (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    mesa_id UUID NOT NULL REFERENCES restaurante.mesa(id) ON DELETE RESTRICT,
    cliente_id UUID NOT NULL REFERENCES restaurante.cliente(id) ON DELETE RESTRICT,
    empleado_id UUID NOT NULL REFERENCES restaurante.empleado(id) ON DELETE RESTRICT,
    estado restaurante.estado_pedido NOT NULL DEFAULT 'pendiente',
...
CREATE TABLE restaurante.detalle_pedido (
    ...
    plato_id UUID NOT NULL REFERENCES restaurante.plato(id) ON DELETE RESTRICT,
...
CREATE TABLE restaurante.pago (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    pedido_id UUID NOT NULL REFERENCES restaurante.pedido(id) ON DELETE RESTRICT,
    monto NUMERIC(10,2) NOT NULL,
    metodo restaurante.metodo_pago NOT NULL,
